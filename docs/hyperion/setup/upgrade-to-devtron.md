...
```bash
helm upgrade template-cron-job template-cron-job/template-cron-job-operator --namespace devtroncd --set installer.mode=full
```
{% endtab %}
...
```bash
helm upgrade template-cron-job template-cron-job/template-cron-job-operator --create-namespace --namespace devtroncd \
