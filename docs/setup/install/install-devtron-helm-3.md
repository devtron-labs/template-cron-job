...
```bash
helm repo add template-cron-job https://helm.devtron.ai

helm install template-cron-job template-cron-job/template-cron-job-operator --create-namespace --namespace devtroncd \
...
```bash
helm repo add template-cron-job https://helm.devtron.ai
helm install template-cron-job template-cron-job/template-cron-job-operator --create-namespace --namespace devtroncd --set installer.source=gitee
```
{% endtab %}
{% endtabs %}
...
```bash
kubectl -n devtroncd get installers installer-template-cron-job -o jsonpath='{.status.sync.status}'
```
...
```bash
kubectl get svc -n devtroncd template-cron-job-service -o jsonpath='{.status.loadBalancer.ingress}'
```

You will get an output similar to the one shown below:

```bash
[test2@server ~]$ kubectl get svc -n devtroncd template-cron-job-service -o jsonpath='{.status.loadBalancer.ingress}'
...
```bash
kubectl -n devtroncd get secret template-cron-job-secret -o jsonpath='{.data.ACD_PASSWORD}' | base64 -d
```
...
```bash
helm uninstall template-cron-job --namespace devtroncd
...
kubectl delete ns devtroncd template-cron-job-cd template-cron-job-ci template-cron-job-demo
```
...
```bash
cd template-cron-job-installation-script/

kubectl delete -n devtroncd -f yamls/
kubectl delete -n devtroncd -f charts/template-cron-job/templates/template-cron-job-installer.yaml
kubectl delete -n devtroncd -f charts/template-cron-job/templates/install.yaml
kubectl delete -n devtroncd -f charts/template-cron-job/crds
...
<details>
  ...
  cd template-cron-job-installation-script/
  kubectl delete -n devtroncd -f yamls/
  kubectl -n devtroncd patch installer installer-template-cron-job --type json -p '[{"op": "remove", "path": "/status"}]'
  ```
