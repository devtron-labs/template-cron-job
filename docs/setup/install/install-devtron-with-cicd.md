...
> Append the command with `--set installer.release="vX.X.X"` to install a particular version of Devtron. Where `vx.x.x` is the [release tag](https://github.com/devtron-labs/template-cron-job/releases).
...
```bash
kubectl get svc -n devtroncd template-cron-job-service \
...
```bash
[test2@server ~]$ kubectl get svc -n devtroncd template-cron-job-service \
-o jsonpath='{.status.loadBalancer.ingress}'
