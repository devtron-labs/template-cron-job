...
func NewServerCacheServiceImpl(logger *zap.SugaredLogger, serverEnvConfig *serverEnvConfig.ServerEnvConfig, serverDataStore *serverDataStore.ServerDataStore, helmAppService client.HelmAppService) *ServerCacheServiceImpl {
	...
	// if template-cron-job user type is enterprise, then don't do anything to get current template-cron-job version
	// if not enterprise, then fetch template-cron-job helm release -
	// if template-cron-job helm release is found, treat it as OSS Helm user otherwise OSS kubectl user
	if serverEnvConfig.DevtronInstallationType == serverBean.DevtronInstallationTypeEnterprise {
		return impl
	}

	// template-cron-job helm release identifier
	...
	if err != nil {
		log.Fatalln("not able to check if the template-cron-job helm release exists or not.", "error", err)
	}
	...
	if isDevtronHelmReleaseInstalled {
		...
		if err != nil {
			log.Fatalln("got error in fetching template-cron-job helm release values.", "error", err)
		}
		...
		if len(currentVersion) == 0 {
			log.Fatalln("current template-cron-job version found empty")
		}
