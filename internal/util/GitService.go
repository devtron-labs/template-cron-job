...
func (impl GitLabClient) CreateReadme(namespace, projectName, userName, userEmailId string) (string, error) {
	actions := &gitlab.CreateCommitOptions{
		...
		Actions:       []*gitlab.CommitAction{{Action: gitlab.FileCreate, FilePath: "README.md", Content: "template-cron-job licence"}},
...
func (impl GitServiceImpl) CommitValues(config *ChartConfig) (commitHash string, err error) {
	...
	hash, err := impl.CommitAndPushAllChanges(gitDir, config.ReleaseMessage, "template-cron-job bot", "template-cron-job-bot@template-cron-job.ai")
	return hash, err
}
